...
public class SysAgentController {
    ...
    public SysAgentController(SysAgentService sysAgentService) {
    ...
    @PostMapping("/create")
    @SaCheckLogin
    public Result<Agent> createAgent(@RequestParam String name) {
        return sysAgentService.createAgent(name);
    }
    ...
    @GetMapping("/get")
    @SaCheckLogin
    public Result<Page<Agent>> getAgent(
            @RequestParam(defaultValue = "1") Long page,
            @RequestParam(defaultValue = "20") Long size
    ) {
        return sysAgentService.getAgent(page, size);
    }
    ...
    @GetMapping("/search")
    @SaCheckLogin
    public Result<Page<Agent>> searchAgent(
            @RequestParam String keyWord,
            @RequestParam(defaultValue = "1") Long page,
            @RequestParam(defaultValue = "20") Long size
    ){
        return sysAgentService.searchAgent(keyWord, page, size);
    }
    ...
    public Result<Agent> updateAgent(
        ...
        return sysAgentService.updateAgent(id, name);
    }
     ...
     */
    @PutMapping("/{agentId}/kbs")
    @SaCheckLogin
    public Result<Void> bindKnowledgeBases(
            @PathVariable Long agentId,
            @RequestBody KbIds kbIds
    ){
        return sysAgentService.bindKnowledgeBases(agentId, kbIds);
    }
     ...
     */
    @GetMapping("/{agentId}/binding-kbs")
    @SaCheckLogin
    public Result<BindingKbInformationVODto> getBindingKbInformation(@PathVariable Long agentId){
        return sysAgentService.getBindingKbInformation(agentId);
    }
    ...
    public Result<Void> deleteAgent(@RequestParam Long id){
        return sysAgentService.deleteAgent(id);
    }
}
